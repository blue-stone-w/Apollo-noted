...
// Generate random integer in half-open interval [0,pool_size), the seed s should not be 0
...
// note that if pool_size is smaller than sample Size n, the sample will contain duplicate integers
inline void IRandomSample(int *sample, int n, int pool_size, int *s)
{
  int temp, curr;
  if (pool_size < n) // if pool_size is smaller than sample Size n
  {
    for (int i = 0; i < n; i++) { sample[i] = IRandI(pool_size, s); }
  }
  else
  {
    for (int i = 0; i < n; i++)
    {
      ...
      // go through previous storted integers, if smaller, store, and move the rest of the array one step.
      // if larger or equal, increase by 1 and go to the next. This ensures even distribution.
      for (int j = 0; j < i; j++)
      {
        curr = sample[j];
        if (temp < curr)
...
{
  if (A == reinterpret_cast<T *>(NULL) || B == reinterpret_cast<T *>(NULL) || n <= 1 || la < 1 || lb < 1)
  {
    return;
  }
  int i, r;
