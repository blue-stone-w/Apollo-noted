...
{
  nr_points_max                 = 320000; // assume max 320000 points; nr: abbreviation of number
...
{
  if (nr_grids_coarse < 2 || nr_grids_fine < 2 || nr_grids_coarse > nr_grids_fine || nr_points_max == 0 || nr_samples_min_threshold == 0 || nr_samples_max_threshold == 0 || nr_inliers_min_threshold == 0 || nr_ransac_iter_threshold == 0 || roi_region_rad_x <= 0.f || roi_region_rad_y <= 0.f || roi_region_rad_z <= 0.f || planefit_dist_threshold_near > planefit_dist_threshold_far)
  {
...
{
  std::vector<std::pair<float, int>> map_dist; // (distance, index of coarse grid)
  float cx       = 0.f;
  float cy       = 0.f;
  float dist2    = 0.f;
  float radius   = 0.f;
  ...
  for (i = 0; i < vg->NrVoxel(); ++i)
  {
    ...
    dist2  = cx * cx + cy * cy;    // square of distance
  ...
  for (i = 0; i < map_dist.size(); ++i)
  {
    ...
    order[i].first    = voxel.iy_; // voxel indices in the Y dimension
    order[i].second   = voxel.ix_; // voxel indices in the X dimension
  }
}
...
bool PlaneFitGroundDetector::Init()
{
  ...
  ground_planes_sphe_ = IAlloc2<GroundPlaneSpherical>(param_.nr_grids_coarse, param_.nr_grids_coarse);
  ...
  for (r = 0; r < param_.nr_grids_coarse; ++r)
  {
    ...
    {
      if (r < (param_.nr_grids_coarse << 3) && c < (param_.nr_grids_coarse << 3)) { capacity = 16384; }
      else if (r < (param_.nr_grids_coarse << 2) && c < (param_.nr_grids_coarse << 2)) { capacity = 8192; }
      else if (r < (param_.nr_grids_coarse << 1) && c < (param_.nr_grids_coarse << 1)) { capacity = 4096; }
      else { capacity = 512; }
  ...
  memset(reinterpret_cast<void *>(pf_threeds_), 0, param_.nr_samples_max_threshold * dim_point_ * sizeof(float));
  // labels:
  ...
  for (r = 0; r < param_.nr_grids_fine; ++r) // row
  {
    pr    = r / sf;
    index = r * param_.nr_grids_fine;
    for (c = 0; c < param_.nr_grids_fine; ++c) // column
    {
      pc                             = c / sf;
                                     ...
                                     unsigned int nr_compares)
{
  ...
  unsigned int nr_z_comp_fail_threshold = IMin(param_.nr_z_comp_fail_threshold, (unsigned int)(nr_compares >> 1));
  std::vector<int>::const_iterator iter = indices.cbegin();
  // Exclude points that are away from other points in height z.
  while (iter < indices.cend())
  {
    ...
    {
      if (z < param_.sample_region_z_lower || z > param_.sample_region_z_upper)
      {
        continue;
      }
    }
...
void PlaneFitGroundDetector::ComputeAdaptiveThreshold()
{
  ...
  float dr       = 0.0f; // 距离中心的行差
  float dc       = 0.0f; // 距离中心的列差
  float k        = 0.0f; // slop: adaptive thireshold changes accorading to distance linearly.
  float b        = 0.0f; // interception
  float min_dist = 0.0f; //
  float max_dist = 0.0f; //
  float thre     = 0.0f; // temporary threshold

  float grid_rad = static_cast<float>(param_.nr_grids_coarse - 1) / 2; // grid radius
  ...
  IMinMaxElements(pf_thresholds_[0], param_.nr_grids_coarse * param_.nr_grids_coarse, &min_dist, &max_dist);
  ...
  {
    k = param_.planefit_dist_threshold_far - param_.planefit_dist_threshold_near; // f-n
    k = k / (max_dist - min_dist);                                                // (f-n)/(max-min)
    b = param_.planefit_dist_threshold_far + param_.planefit_dist_threshold_near; // f+n
    b = (b - k * (min_dist + max_dist)) / 2;                                      // (n*max-f*min)/(max-min)
    for (r = 0; r < param_.nr_grids_coarse; ++r)
    {
      for (c = 0; c < param_.nr_grids_coarse; ++c)
      {
...
void PlaneFitGroundDetector::ComputeSignedGroundHeight(const float *point_cloud, float *height_above_ground,
                                                       unsigned int nr_points, unsigned int nr_point_elements)
{
  ...
  ComputeSignedGroundHeightLine(point_cloud, ground_planes_[0], ground_planes_[0], ground_planes_[1],
                                height_above_ground, 0, nr_points, nr_point_elements);
  for (r = 1; r < nm1; ++r)
  {
    ComputeSignedGroundHeightLine(point_cloud, ground_planes_[r - 1], ground_planes_[r], ground_planes_[r + 1],
                                  height_above_ground, r, nr_points, nr_point_elements);
  }
  ComputeSignedGroundHeightLine(point_cloud, ground_planes_[nm1 - 1], ground_planes_[nm1], ground_planes_[nm1],
                                height_above_ground, nm1, nr_points, nr_point_elements);
}

void PlaneFitGroundDetector::ComputeSignedGroundHeightLine(const float *point_cloud, const GroundPlaneLiDAR *up,
...
{
  unsigned int i       = 0;       //
  unsigned int c       = 0;       // col
  unsigned int id      = 0;       //
  int pos              = 0;       // index of point in current pc
  char label           = 0;       // 该点是否在高度上远离其它点
  const float *cptr    = nullptr; // 指向每个点的数据块的首地址
  ...
  while (iter < (*vg_coarse_)(r, 0).indices_.cend())
  {
    ...
    dist[0]      = plane[0] != nullptr ? IPlaneToPointSignedDistanceWUnitNorm(plane[0], cptr) : std::numeric_limits<float>::max();
    min_abs_dist = IAbs(dist[0]);
    id           = 0;
    // for candidates we take min dist:
    if (label)
    {
      dist[1] = plane[1] != nullptr ? IPlaneToPointSignedDistanceWUnitNorm(plane[1], cptr) : std::numeric_limits<float>::max();
      dist[2] = plane[2] != nullptr ? IPlaneToPointSignedDistanceWUnitNorm(plane[2], cptr) : std::numeric_limits<float>::max();
      dist[3] = plane[3] != nullptr ? IPlaneToPointSignedDistanceWUnitNorm(plane[3], cptr) : std::numeric_limits<float>::max();
      for (i = 1; i < 4; ++i)
      {
    ...
    ++iter;
  } // endwhile: done: r-th row, o-th col
  ...
  for (c = 1; c < nm1; ++c)
  {
    ...
    while (iter < (*vg_coarse_)(r, c).indices_.cend())
    {
      ...
      if (label)
      {
        ...
        dist[2] = plane[2] != nullptr ? IPlaneToPointSignedDistanceWUnitNorm(plane[2], cptr) : std::numeric_limits<float>::max();
        dist[3] = plane[3] != nullptr ? IPlaneToPointSignedDistanceWUnitNorm(plane[3], cptr) : std::numeric_limits<float>::max();
        dist[4] = plane[4] != nullptr ? IPlaneToPointSignedDistanceWUnitNorm(plane[4], cptr) : std::numeric_limits<float>::max();
        for (i = 1; i < 5; ++i)
        {
          if (min_abs_dist > IAbs(dist[i]))
          {
      ...
      height_above_ground[pos] = dist[id];
      ++iter;
    } // endwhile: done a coarse grid
  }   // endfor: done all but last coarse grids
  ...
  while (iter < (*vg_coarse_)(r, nm1).indices_.cend())
  {
    ...
    height_above_ground[pos] = dist[id];
    ++iter;
  } // endwhile: done last coarse grid
}
...
int PlaneFitGroundDetector::FilterGrid(const Voxel<float> &vx,
                                       ...
                                       unsigned int nr_point_element)
{
  int pos        = 0;              // 用作内存的地址，存放每个点的数据块的起始位置
  int rseed      = I_DEFAULT_SEED; // 一个用于产生随机数的值
  int nr_candis  = 0;              // 每个grid中，排除高度上的离散点后剩余的点的数量
  unsigned int i = 0;
  // how many points will be reserved in this grid
  ...
  // 如果当前细粒度体素网格内的点数量≥nr_z_comp_candis，留下所有的点。
  // 如果当前细粒度体素网格内的点数量＞nr_z_comp_candis，随机留下nr_z_comp_candis个点。
  if (vx.NrPoints() <= param_.nr_z_comp_candis)
  {
    ...
    {
      pos = vx.indices_[i] * nr_point_element; // 点的序号*每个点的元素数量(XYZ点的数量为3)
      // requires the Z element to be in the third position, i.e., after X, Y
      // 数组名和数组的索引本质都是是记录地址偏移：point_cloud表示点云数组的起始地址(从地址0开始的地址偏移)，
      // pos和2：表示继续偏移poshe2个地址。这里的地址偏移的单位是元素的大小
      sampled_z_values_[i] = (point_cloud + pos)[2];
    }
  }
  else
  { // (out index of points, in sample size, in num of all points, in num to generate a random integer)
    IRandomSample(sampled_indices_, static_cast<int>(param_.nr_z_comp_candis), static_cast<int>(vx.NrPoints()), &rseed);
    //  sampled z values
  ...
  // here we get index and z of sampled points
  // Filter points and get plane fitting candidates
  nr_candis = CompareZ(point_cloud, vx.indices_, sampled_z_values_, candi, nr_points, nr_point_element, nr_samples);
  return nr_candis;
}

int PlaneFitGroundDetector::FilterLine(unsigned int r) // filer r-th row
{
  int nr_candis  = 0; // 排除高度上的离散点后剩余的点的数量
  unsigned int c = 0; // 在循环中用作列号
  int parent     = 0; // index of coarse grid that this fine grid locates in.

  const float *point_cloud      = vg_fine_->const_data();     // parse pc
  unsigned int nr_points        = vg_fine_->NrPoints();       // num of points in this grid
  unsigned int nr_point_element = vg_fine_->NrPointElement(); // 一个点含有的元素数量，例如有XYZ三个元素
  unsigned int begin            = (r * param_.nr_grids_fine); // 这条线起始 fine grid 的序号
  // traverse every column in this row
  for (c = 0; c < param_.nr_grids_fine; c++)
  {
    parent = map_fine_to_coarse_[begin + c];
    // (in a grid, ori pc in this cloud, )
    nr_candis += FilterGrid((*vg_fine_)(r, c), point_cloud, &local_candis_[0][parent], nr_points, nr_point_element);
  }
  return nr_candis;
}
...
{
  int nr_candis  = 0; // 去除高度上离散的点后剩余的点的数量
  unsigned int i = 0;
  unsigned int r = 0; // 在循环中用作行号 index of row
  /*
  ...
  */
  ...
  //  Filter plane fitting candidates; traverse all rows
...
int PlaneFitGroundDetector::FilterCandidates(int r, int c, const float *point_cloud, PlaneFitPointCandIndices *candi,
                                             ...
                                             unsigned int nr_point_element)
{
  ...
  if (count > 0)
  {
    ...
    for (i = 0; i < candi->Size(); ++i)
    {
      ...
      if (z > avg_z - param_.candidate_filter_threshold && z < avg_z + param_.candidate_filter_threshold)
      {
...
int PlaneFitGroundDetector::FitGridWithNeighbors(int r, int c, const float *point_cloud,
                                                 GroundPlaneLiDAR *groundplane,
                                                 ...
                                                 float dist_thre)
{
  ...
  std::vector<std::pair<int, int>> neighbors; // pair<row,col>, neighbor grid
  // (in cur row, in cur col, in num of row, in num of col, out row&col of neighbor grid)
  GetNeighbors(r, c, param_.nr_grids_coarse, param_.nr_grids_coarse, &neighbors);
  // (in cur row, in cur col, out ***, in row&col of neighbor grid, in)
  ...
  GroundPlaneLiDAR hypothesis[kNr_iter]; // generate k hypothesises

  float ptp_dist      = 0.0f; // distance between point and plane
  int best            = 0;    // index of best plane
  int nr_inliers      = 0;    // num of points in this plane
  int nr_inliers_best = -1;   // num of points in best plane
  ...
  float samples[9]; // 3 sampled points: 3x3 matrix stores: x, y, z; x, y, z; x, y, z;
  // copy 3D points
  float *psrc = nullptr;
  float *pdst = pf_threeds_;
  int r_n     = 0;
  ...
  {
    assert(candi[i] < static_cast<int>(nr_points));
    ICopy3(point_cloud + (nr_point_element * candi[i]), pdst); // (in, out)
    pdst += dim_point_;                                        // move pointer
  }
  // generate plane hypothesis and vote
  ...
  } // endfor: have found all likely planes in this grid

  // check whether neighbors' planes are also for this grid
  ...
  } // endfor: have checked whether neighbors' planes are also for this grid
  ...
  for (int i = 0; i < kNr_iter; ++i)
  {
    ...
    // use 2 conditions to evaluate: num of points in this plane and angle diff with neighbors' planes
  ...
  *groundplane = hypothesis[best];
  ...
  // note that pf_threeds_ will be destroyed after calling this routine
  // (in points, out optimal plane, in num of points)
  IPlaneFitTotalLeastSquare(pf_threeds_, groundplane->params, nr_inliers);
  ...
  float cz     = -(groundplane->params[0] * cx + groundplane->params[1] * cy + groundplane->params[3]) / groundplane->params[2];
...
float PlaneFitGroundDetector::CalculateAngleDist(const GroundPlaneLiDAR &plane,
...
int PlaneFitGroundDetector::FitInOrder()
{
  ...
  int c          = 0;
  GroundPlaneLiDAR gp;
  // initialize z of all coarse grids
...
void PlaneFitGroundDetector::GetNeighbors(int r, int c, int rows, int cols, std::vector<std::pair<int, int>> *neighbors)
{
  int left  = IMax(0, c - 1);
...
int PlaneFitGroundDetector::SmoothLine(unsigned int up, unsigned int r, unsigned int dn) // (r-1,r,r+1)
{
  int nr_grids     = 0;
  unsigned int c   = 0;
  unsigned int nm1 = param_.nr_grids_coarse - 1;
  ...
  { // 如果此grid没有在之前的计算中得到有效地面平面，就用周围grid的地面平面插补得到
...
int PlaneFitGroundDetector::CompleteGrid(const GroundPlaneSpherical &lt, const GroundPlaneSpherical &rt,
                                         ...
                                         GroundPlaneSpherical *gp)
{
  ...
  // weighted average:
  gp->theta = weights[0] * lt.theta + weights[1] * rt.theta + weights[2] * up.theta + weights[3] * dn.theta;
  ...
  support_sum = IMax(((support_sum + 2) >> 2), 1); // "+2" is to round to nearest int
                                       ...
                                       GroundPlaneSpherical *gp)
{
  ...
  // weighted average:
  gp->theta = weights[0] * lt.theta + weights[1] * rt.theta + weights[2] * up.theta + weights[3] * dn.theta + weights[4] * g.theta;
  gp->phi   = weights[0] * lt.phi + weights[1] * rt.phi + weights[2] * up.phi + weights[3] * dn.phi + weights[4] * g.phi;
  gp->d     = weights[0] * lt.d + weights[1] * rt.d + weights[2] * up.d + weights[3] * dn.d + weights[4] * g.d;
...
bool PlaneFitGroundDetector::Detect(const float *point_cloud,       // 点云信息
                                    float *height_above_ground,     // 返回每个点距离地面的距离
                                    unsigned int nr_points,         // 点云的数量
                                    unsigned int nr_point_elements) // 点数
{
  ...
  // setup the fine voxel grid 设置精细的体素网格
  if (!vg_fine_->SetS(point_cloud, nr_points, nr_point_elements))
  {
    return false;
  }
  // setup the coarse voxel grid 设置粗体素网格
  ...
  unsigned int r = 0;
  unsigned int c = 0;
  // Filter to generate plane fitting candidates 过滤以生成候选平面拟合
  // int nr_candis =
  Filter();
  // std::cout << "# of plane candidates: " << nr_candis << std::endl;
  //  Fit local plane using ransac 使用ransac拟合面
  // nr_valid_grid = Fit();
  // int nr_valid_grid =
  FitInOrder();
  // std::cout << "# of valid plane geometry (fitting): " << nr_valid_grid <<
  // std::endl;
  // Smooth plane using neighborhood information: 使用邻域信息平滑平面：
  ...
  // std::endl;
  // compute point to ground distance 计算点到地面的距离
