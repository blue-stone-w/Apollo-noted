                               ...
                               int nr_voxel_y)
{
  ...
  if (x < bound_x_min || x > bound_x_max || y < bound_y_min || y > bound_y_max || z < bound_z_min || z > bound_z_max)
  {
    return (-1);
  }
...
class VoxelGridXY
{
 ...
 private:
  ...
  unsigned int nr_point_element_ = 0; // 一个点的大小，比如PointXYZ，这个大小就是3
...
bool VoxelGridXY<T>::AllocAlignedMemory()
{
  ...
  return (mem_aligned16_f32_ != nullptr && mem_aligned16_i32_ != nullptr && IVerifyAlignment(mem_aligned16_f32_, 4) && IVerifyAlignment(mem_aligned16_i32_, 4));
}
...
int VoxelGridXY<T>::WhichVoxel(T x, T y, T z) const
{
  ...
  if (x < dim_x_[0] || x > dim_x_[1] || y < dim_y_[0] || y > dim_y_[1] || z < dim_z_[0] || z > dim_z_[1])
  {
                         ...
                         T spatial_bound_z_max, bool force_bound)
{
  ...
  for (; n < nr_points_; n++, nd += nr_point_element_)
  {
    ...
    if (x < dim_x_min || x > dim_x_max || y < dim_y_min || y > dim_y_max || z < dim_z_min || z > dim_z_max)
    {
      // points that are outside the defined BBOX are ignored
      continue;
    }
                            ...
                            unsigned int dsf_dim_x, unsigned int dsf_dim_y)
{
  ...
  dim_x_dst[1] = (0 == nr_res_voxel_x_dst) ? dim_max_x_src : (voxel_width_x_dst * nr_voxel_x_dst + dim_x_dst[0]);
  dim_y_dst[1] = (0 == nr_res_voxel_y_dst) ? dim_max_y_src : (voxel_width_y_dst * nr_voxel_y_dst + dim_y_dst[0]);
