...
struct GroundPlaneLiDAR
{
  ...
  // degree between normal and z-axis
  float GetDegreeNormalToZ() const
  {
    ...
    // normalize:
    IScale3(normal, IRec(ISqrt(ISquaresum3(normal)))); // 归一化
  ...
  float params[4]; // ax+by+cz+d = 0: vector[a, b, c, d]
...
struct PlaneFitGroundDetectorParam
{
  ...
  // A square is devided into nr_grids_fine*nr_grids_fine grids.
  unsigned int nr_grids_fine;            // side length of this square
  unsigned int nr_grids_coarse;          //
  unsigned int nr_z_comp_candis;         // num of points that are sample in every fine grid
...
{
  static const int dim_point_ = 3; // dimension
 ...
 protected:
  ...
  int FilterGrid(const Voxel<float> &vg, const float *point_cloud,
                 ...
                 unsigned int nr_point_element);
  int Smooth();
  int SmoothLine(unsigned int up, unsigned int r, unsigned int dn);                // (上一行，当前行，下一行)
  int CompleteGrid(const GroundPlaneSpherical &lt, const GroundPlaneSpherical &rt, // 左右相邻格
                   const GroundPlaneSpherical &up, const GroundPlaneSpherical &dn, // 上下相邻格
                   GroundPlaneSpherical *gp);
  int SmoothGrid(const GroundPlaneSpherical &g,                                  // 当前
                 const GroundPlaneSpherical &lt, const GroundPlaneSpherical &rt, // 左右相邻格
                 const GroundPlaneSpherical &up, const GroundPlaneSpherical &dn, // 上下相邻格
                 GroundPlaneSpherical *gp);
 ...
 protected:
  VoxelGridXY<float> *vg_fine_; // array whose element is set of points in this grid. Element also contains other info.
  ...
  unsigned int *map_fine_to_coarse_;          // index is index of fine grid; value is index of coarse grid
  char *labels_;                              // 记录每个点在高度上是否远离其它点: 1(不远离) 0(远离)
  float *sampled_z_values_;                   // 采样的到的点的z
  float *pf_threeds_;                         // 采样的到的点的索引
  int *sampled_indices_;
  std::pair<int, int> *order_table_; // voxel indices in the X, Y dimensions
};
